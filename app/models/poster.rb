...
class Poster
  ...
  def batch_publish
    providers.each do |provider|
      if logins[provider]
        @author = Identity.where(provider: provider,
                                 uid: logins[provider]).first
        status[provider] = instance_eval("post_to_#{provider}(body)")
      else
        status[provider] = "You cannot post on #{pretty_provider_name(provider)}
                        until you log in. To log in, just click the link above."
      end
    end
  end
  ...
  POST_METHODS.keys.each do |provider|
    class_eval %(
      def post_to_#{provider}(post)
        client = set_up_#{provider}_client
        begin
          outcome = 'Success!' if client.#{POST_METHODS[provider]}
        rescue
          outcome = "Error. Try again or try posting directly on
                     #{provider.to_s.humanize} website."
        end
        outcome
      end
    )
  ...
  def set_up_salesforce_client
    ...
    client.version = '23.0'
    Databasedotcom::Chatter::User.find(client, 'me')
  end

  def set_up_facebook_client
    Koala::Facebook::API.new(author.oauth_token)
  end
